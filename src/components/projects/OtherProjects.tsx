...
export const OtherProjects = () => {
	...
	useEffect(() => {
		const fetchAllRepos = async () => {
			...
			while (hasMore) {
				const response = await fetch(`https://api.github.com/users/santiagobaronz/repos?page=${page}`);
			...
			const filteredData = allRepos.filter(
				project => project.topics?.includes('deployed') && project.homepage
			);
			...
			const cleanedProjects = filteredData.map(project => ({
				...project,
				homepage: project.homepage.replace(/^https?:\/\//, ''),
			}));

			setProjects(cleanedProjects);
			setIsLoading(false);
			setIsEmpty(cleanedProjects.length === 0);
		};
	...
	if (isEmpty) {
		return (
			<div className='flex items-center justify-center w-full h-full bg-gray-hover text-center rounded-lg py-5 text-gray'>
				<p className='px-10'>No hay proyectos disponibles para mostrar</p>
			</div>
		);
	}
	...
	if (isLoading) {
		return (
			<div className='flex items-center justify-center w-full h-full bg-gray-hover text-center rounded-lg py-5 text-gray'>
				<p className='animate-pulse px-10'>Cargando proyectos...</p>
			</div>
		);
	}
		...
		<>
			{projects.map(project => (
				<li className="xl:flex xl:flex-wrap w-full bg-gray-hover xl:mb-4 rounded-xl items-center xl:h-20 h-auto xl:px-10 xl:py-0 px-10 py-10 hover:translate-x-0.5 hover:-translate-y-0.5 transition-all" key={project.id}>
					...
					<div className="xl:w-[27%] w-full xl:p-2 flex items-center mb-3 xl:mb-0" title='Última actualización'>
						...
						<p className="text-gray-600 text-sm text-black-300 ml-4 date-project">{getDate(project.pushed_at)}</p>
					</div>
					...
					<div className="flex items-center justify-end">
						<span className="w-full bg-[#32d49e43] rounded-md px-6 text-center py-2 text-[#32d49e] text-sm font-semibold deployed">Activo</span>
					</div>
				</li>
			))}
		</>
