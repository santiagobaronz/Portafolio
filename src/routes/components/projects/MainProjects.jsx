	...
	return (
		<>
			{projects.map(project => (
				<li className="group relative flex flex-col items-start" key={project.id}>
					<div
						...
						<img src={project.icono_proyecto} width="32" height="32" decoding="async" data-nimg="1" className="h-8 w-8"
							loading="lazy" />
					</div>
					<h2 className="mt-6 text-base font-semibold text-white">
						<div
							...
							className="absolute -inset-y-6 -inset-x-4 z-20 sm:-inset-x-6 sm:rounded-2xl"></span><span
								className="relative z-10">{project.nombre_proyecto}</span></a>
					</h2>
					<p className="relative z-10 mt-2 text-sm text-black-300">{project.descripcion_proyecto}</p>
