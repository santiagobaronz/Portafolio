export const fillArticles = async () => {
    ...
    articlesArray.forEach(article => {
        ...
        const objectList = document.createElement("article");
        objectList.className = "md:grid md:grid-cols-4 md:items-baseline";
        objectList.innerHTML =  `
        <div class="md:col-span-3 group relative flex flex-col items-start">
            <h2 class="text-base font-semibold tracking-tight text-zinc-800 dark:text-zinc-100">
                <div class="absolute -inset-y-6 -inset-x-4 z-0 scale-95 bg-gray-hover opacity-0 transition group-hover:scale-100 group-hover:opacity-100 sm:-inset-x-6 sm:rounded-2xl">
        ...
        <time class="mt-1 md:block relative z-10 order-first mb-3 flex items-center text-sm text-black-300" datetime="2022-09-05">
            ${article.fecha_texto}
        </time>
        `;
